...
import { Search, Sparkles, LayoutGrid, ChevronRight, ChevronDown, Store } from 'lucide-react';
...
export default function CatalogPage() {
  ...
  // Sincroniza dinámicamente las categorías del árbol basándose en la búsqueda activa
  useEffect(() => { 
    fetchActiveCategories(); 
    setPage(0); // Reiniciar página al buscar
  }, [search]);

  // Refresca los productos al cambiar de categoría, página o búsqueda
  useEffect(() => { 
    fetchProducts(); 
  }, [selectedCategory, page, search]);

  // Construye la estructura de árbol garantizando unicidad mediante el path completo (fullName)
  ...
  // Trae las categorías únicamente de los productos válidos y visibles que cumplan con la búsqueda actual
  async function fetchActiveCategories() {
    let query = supabase.from('products').select('category').eq('is_visible', true);
    
    // Regla unificada general: Locales activos OR (Scraping con precio de costo original válido)
  ...
  async function fetchProducts() {
    ...
    // Query Base General (Sin separación por pestañas locales o scraping)
    let query = supabase.from('products').select('*').eq('is_visible', true);
    query = query.or('is_local.eq.true,and(price_original.gt.0,or(is_local.is.null,is_local.eq.false))');
    
    // Filtro de Categoría Lateral Unificado
  ...
  const toggleExpand = (fullName: string) => {
    setExpandedCats(prev => ({ ...prev, [fullName]: !prev[fullName] }));
  };

  const RenderCategory = ({ node, level = 0 }: { node: CategoryNode, level: number }) => {
    const hasChildren = Object.keys(node.children).length > 0;
    // CORREGIDO: Se evalúa utilizando node.fullName en vez de node.name para evitar colisión de nombres duplicados
    ...
    return (
      <div className="flex flex-col">
        ...
        >
          <div onClick={(e) => { e.stopPropagation(); toggleExpand(node.fullName); }} className="p-1 hover:bg-white/5 rounded">
    ...
    <div className="min-h-screen bg-[#030303] text-white">
      ...
      <div className="max-w-7xl mx-auto px-6 py-12 flex flex-col md:flex-row gap-12">
        <aside className="w-full md:w-64 flex-shrink-0">
          {/* SE REMOVIÓ POR COMPLETO LA SECCIÓN DE PESTAÑAS (LOCAL / SCRAPING) */}
          ...
          <div className="space-y-1">
            <button 
              ...
              className={`text-[11px] font-bold uppercase tracking-wider py-2 w-full text-left transition-colors ${selectedCategory === "All" ? 'text-emerald-500' : 'text-gray-400 hover:text-white'}`}
            >
              TODOS LOS ARTÍCULOS
            </button>
                ...
                return (
                  <Link href={`/products/${product.id}`} key={product.id} className="group relative">
                    <div className="relative aspect-[4/5] overflow-hidden bg-[#0a0a0a] border border-white/5 group-hover:border-emerald-500/30 transition-all duration-500">
                      ...
                      {/* Badge informativo de stock disponible */}
                      {product.is_local && (
                    ...
                    <div className="mt-6 space-y-1">
                      <div className="flex justify-between items-start">
                        <span className="text-[8px] text-emerald-500 font-black uppercase tracking-widest">
                          {product.category?.split('>').pop()?.trim() || "General"}
                        </span>
                      </div>
