...
export default function OrderDetail() {
  ...
  const updateStatus = async (newStatus: 'pending' | 'confirmed' | 'arrived' | 'delivered') => {
    ...
    // Guardamos el estado anterior por si la base de datos falla (rollback)
    const previousStatus = order?.status;
    
    // Actualización optimista: Cambiamos el estado visual inmediatamente para romper la lentitud
    ...
    try {
      ...
      if (!error && data) {
        // Seteamos los datos finales devueltos por la base de datos
        setOrder(data);
      } else {
        console.error("Supabase update error:", error);
        alert("Error al actualizar el estado del pedido");
        // Revertimos al estado anterior si hubo un error en la BD
  ...
  const exchangeRate = order.items?.applied_exchange_rate || 7500;

  const totalAmountNum = Number(order.total_amount) || 0;

  const totalCostPYG = isAdmin 
    ? productList.reduce((acc: number, item: any) => acc + (((item.cost_at_purchase || 0) * exchangeRate) * (item.quantity || 1)), 0) 
    : 0;
  const totalProfitPYG = isAdmin ? (totalAmountNum - totalCostPYG) : 0;

  return (
    <div className="min-h-screen bg-[#050505] text-white pt-20 px-6 pb-20">
      <div className="max-w-5xl mx-auto">
        
        {/* HEADER DEL PEDIDO */}
        <div className="bg-[#111] border border-white/5 p-8 rounded-[2.5rem] mb-8 relative overflow-hidden">
          {isAdmin && (
            <div className="absolute top-6 right-6 flex items-center gap-2 bg-emerald-500/10 text-emerald-500 px-4 py-2 rounded-full border border-emerald-500/20">
              <AdminIcon size={14}/>
              <span className="text-[10px] font-black uppercase tracking-tighter">Panel de Control Admin</span>
            </div>
          )}
          ...
          <div className="flex items-center gap-4 mb-6">
            ...
            <div>
              <h1 className="text-2xl font-black italic uppercase tracking-tighter">Resumen del Pedido</h1>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 pt-6 border-t border-white/5">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-[9px] text-gray-500 uppercase font-bold">Estado Actual</p>
        ...
        {/* TABLA DE PRODUCTOS */}
        <div className="bg-[#111] border border-white/5 rounded-[2.5rem] overflow-hidden mb-8">
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-white/5 text-[10px] font-black uppercase tracking-widest text-gray-400">
                  ...
                  <th className="p-6 text-right">Precio Total (Venta)</th>
                  {isAdmin && <th className="p-6 text-right text-red-400">Costo Base (Admin)</th>}
                  {isAdmin && <th className="p-6 text-right text-cyan-400">Diferencia (Ganancia)</th>}
                </tr>
              </thead>
              <tbody className="divide-y divide-white/5">
				  {productList.map((item: any, idx: number) => {
					...
					// Tratamos el costo como valor directo en Guaraníes, sin conversiones
					...
					return (
					  <tr key={idx} className="hover:bg-white/[0.02] transition-colors">
						<td className="p-6 flex items-start gap-4">
						  <div className="w-16 h-16 bg-white/5 border border-white/5 rounded-2xl overflow-hidden flex-shrink-0 p-2">
							{item.image ? (
						  ...
						  <div className="space-y-1">
							...
							{isAdmin && item.url_original && (
							  <div className="pt-1">
								<a 
								  href={item.url_original} 
								  target="_blank" 
								  rel="noopener noreferrer"
								  className="inline-block text-[9px] bg-white/10 hover:bg-emerald-500 hover:text-black px-2 py-0.5 rounded transition-all font-black uppercase"
								>
								  Ver Original ↗
								</a>
							  </div>
						...
						{isAdmin && (
						  <td className="p-6 text-right bg-red-500/5">
						...
						{isAdmin && (
						  <td className="p-6 text-right bg-cyan-500/5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-start">
          ...
          {/* PANEL DE CONTROL LOGÍSTICO COMPLETO */}
          {isAdmin && (
            <div className="bg-[#111] border-2 border-dashed border-white/10 p-8 rounded-[2.5rem]">
              ...
              <div className="flex flex-col gap-3">
                ...
                >
                  <span className="flex items-center gap-2">
                    <CheckCircle2 size={16}/> 1. Confirmar Pedido
                  </span>
                  {order.status === 'confirmed' && <span className="text-[9px] bg-black/10 px-2 py-0.5 rounded font-bold">ACTIVO</span>}
                </button>

                {/* Paso 2: Marcar como Arribado */}
                <button 
                ...
                >
                  <span className="flex items-center gap-2">
                    <Truck size={16}/> 2. Marcar como Arribado (CDE)
                  </span>
                  {order.status === 'arrived' && <span className="text-[9px] bg-black/10 px-2 py-0.5 rounded font-bold">ACTIVO</span>}
                </button>
                ...
                >
                  <span className="flex items-center gap-2">
                    <ArrowRight size={16}/> 3. Marcar como Entregado al Cliente
                  </span>
                  {order.status === 'delivered' && <span className="text-[9px] bg-white/20 px-2 py-0.5 rounded font-bold">COMPLETADO</span>}
                </button>

                {/* Botón de reinicio rápido en caso de necesitar correcciones de back-office */}
                {order.status !== 'pending' && (
                  <button 
                    onClick={() => updateStatus('pending')}
                    disabled={updating}
                    className="text-left text-[9px] text-gray-600 hover:text-gray-400 transition-colors uppercase font-bold tracking-wider px-2 pt-2"
                  >
                ...
                {order.status === 'delivered' && (
                  <div className="p-4 bg-blue-500/10 border border-blue-500/20 rounded-2xl text-center text-blue-400 font-bold text-xs uppercase tracking-wider mt-2">
                    Este pedido se ha completado y entregado con éxito.
                  </div>
                )}
                ...
                >
                  <XCircle size={16}/> Cancelar y Eliminar Pedido
                </button>
              </div>
            </div>
          )}
          ...
          <div className={`space-y-4 ${!isAdmin ? 'md:col-start-2' : ''}`}>
            {isAdmin && (
              <div className="bg-cyan-500/10 border border-cyan-500/20 p-6 rounded-3xl flex justify-between items-center">
                <div className="flex items-center gap-3">
                  <div className="p-3 bg-cyan-500/20 rounded-xl text-cyan-400">
                    <TrendingUp size={20} />
                  </div>
                  <div>
                    <p className="text-[9px] text-cyan-500 font-black uppercase tracking-wider">Margen Neto de Ganancia</p>
                    <p className="text-xl font-black text-cyan-400 italic">{formatPYG(totalProfitPYG)}</p>
                  </div>
                </div>
                <div className="text-right">
                  <p className="text-[9px] text-gray-500 uppercase font-bold">Total USD</p>
                  <p className="text-sm font-bold text-gray-400">${(totalProfitPYG / exchangeRate).toFixed(2)}</p>
                </div>
              </div>
            )}

            <div className="bg-emerald-500 p-8 rounded-[2.5rem] text-black shadow-[0_20px_50px_-12px_rgba(16,185,129,0.4)]">
              <p className="text-[10px] font-black uppercase tracking-widest mb-1 opacity-60">Total a Pagar en Guaraníes</p>
              ...
              <p className="text-[9px] font-bold uppercase mt-4 border-t border-black/10 pt-4 opacity-50">
                Mochi Football - Gestión de Carga
              </p>
            </div>
          </div>
        </div>
